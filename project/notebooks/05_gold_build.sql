...
COMMENT ON COLUMN statestreet.g_statestreet.securities_master.net_settlement_amount IS
  'Net cash settlement amount for BOND and MUNI security types. Formula: current_face_value x (1 + coupon_rate). NULL for all other product types and for bonds with no coupon record. Precision: DECIMAL(18,6). Currency follows bond_face_currency_code.';
